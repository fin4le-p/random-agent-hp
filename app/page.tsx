  ...
  return (
    <main className="flex min-h-screen flex-col">
      ...
      <div className="mx-auto flex w-full max-w-5xl flex-1 flex-col px-4 pb-10 pt-6 md:pt-10">
        ...
        {/* ===== SECTION: AI ===== */}
        <section className="mb-12 rounded-3xl border border-emerald-400/20 bg-gradient-to-br from-emerald-500/10 via-black/60 to-black/40 px-5 py-8 md:px-8">
          <SectionHeader
            kicker="AI COMMANDS"
...
function AiCommandRow({
  ...
  return (
    <div className="flex flex-col gap-1 rounded-xl border border-emerald-400/25 bg-black/70 px-3 py-2">
      <div className="flex items-center justify-between gap-2">
        <span className="font-mono text-[11px] text-emerald-50">{command}</span>
        <span className="rounded-full border border-emerald-400/50 bg-emerald-400/10 px-2 py-0.5 text-[10px] tracking-[0.16em] text-emerald-200">
          {label}
        </span>
      </div>
      <p className="text-[11px] text-emerald-100/85">{desc}</p>
    </div>
  );
}

function AiModelCard({ title, desc }: { title: string; desc: string }) {
  return (
    <div className="rounded-xl border border-emerald-400/25 bg-black/70 px-3 py-2 text-emerald-100/85">
      <p className="font-mono text-[11px] text-emerald-50">{title}</p>
      <p className="mt-1 text-[11px] text-emerald-100/80">{desc}</p>
    </div>
  );
}
...
function AiFeatureImageCard({
  ...
  return (
    <article className="group grid gap-5 rounded-2xl border border-emerald-400/20 bg-black/60 p-4 shadow-[0_16px_45px_rgba(0,0,0,0.7)] transition hover:-translate-y-1 hover:shadow-[0_20px_55px_rgba(0,0,0,0.85)] md:grid-cols-2">
      <div className="relative overflow-hidden rounded-xl border border-emerald-400/20">
        <Image
          src={image}
          alt={title}
          width={800}
      ...
      <div className="flex flex-col justify-center space-y-2">
        <span className="inline-flex w-fit items-center rounded-full border border-emerald-400/60 bg-emerald-400/10 px-2 py-0.5 text-[10px] tracking-[0.14em] text-emerald-200">
          {tag}
        </span>

        <div className="font-mono text-[11px] text-emerald-200/90">{command}</div>

        <h3 className="text-lg font-semibold text-slate-50">{title}</h3>

        <p className="text-sm leading-relaxed text-emerald-100/90">{text}</p>
      </div>
    </article>
  );
}
